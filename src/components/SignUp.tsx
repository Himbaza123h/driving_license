...
const SignUp: React.FC<SignUpProps> = ({ onSwitchToSignIn }) => {
  ...
  const validateNationalId = (nationalId: string) => {
    ...
    if (!/^[0-9]+$/.test(nationalId)) return { isValid: false, message: 'National ID must contain only numbers' };
    if (nationalId.length < 13) return { isValid: false, message: 'National ID must be at least 13 digits' };
    if (nationalId.length > 16) return { isValid: false, message: 'National ID cannot exceed 16 digits' };
    return { isValid: true, message: '' };
  };

  const validatePhoneNumber = (phoneNumber: string) => {
    if (!phoneNumber || phoneNumber === '+257 ') return { isValid: false, message: '' };
    const phoneRegex = /^\+257 [0-9]{2} [0-9]{3} [0-9]{3}$/;
    if (!phoneRegex.test(phoneNumber)) return { isValid: false, message: 'Phone number must be in format +257 XX XXX XXX' };
    return { isValid: true, message: '' };
  };
  ...
  const validateForm = (): boolean => {
    if (!formData.name || !formData.email || !formData.nationalId || !formData.phoneNumber || !formData.password || !formData.confirmPassword) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await signUp(formData.email, formData.password, formData.name, formData.nationalId, formData.phoneNumber);
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="relative">
            <input
              ...
              className={`w-full px-4 py-3 pr-12 border rounded-lg focus:ring-2 focus:ring-[#2C8E5D] focus:border-transparent transition-all duration-200 font-inter text-gray-900 placeholder-gray-500 ${
                formData.name ? 
                  (nameValidation.isValid ? 'border-green-500' : 'border-red-500') 
                  : 'border-gray-300'
              }`}
            ...
            {formData.name && (
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                <FontAwesomeIcon 
                  ...
                  className={`w-5 h-5 ${nameValidation.isValid ? 'text-green-500' : 'text-red-500'}`} 
                />
              </div>
            )}
          </div>
            ...
            {formData.email && (
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                <FontAwesomeIcon 
                  ...
                  className={`w-5 h-5 ${emailValidation.isValid ? 'text-green-500' : 'text-red-500'}`} 
                />
              </div>
            )}
          </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className={`w-full px-4 py-3 pr-12 border rounded-lg focus:ring-2 focus:ring-[#2C8E5D] focus:border-transparent transition-all duration-200 font-inter text-gray-900 placeholder-gray-500 ${
                formData.nationalId ? 
                  (nationalIdValidation.isValid ? 'border-green-500' : 'border-red-500') 
                  : 'border-gray-300'
              }`}
            ...
            {formData.nationalId && (
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                <FontAwesomeIcon 
                  ...
                  className={`w-5 h-5 ${nationalIdValidation.isValid ? 'text-green-500' : 'text-red-500'}`} 
                />
              </div>
            )}
          </div>
          {formData.nationalId && !nationalIdValidation.isValid && (
            <p className="mt-1 text-sm text-red-600">{nationalIdValidation.message}</p>
          )}
          {formData.nationalId && nationalIdValidation.isValid && (
            <p className="mt-1 text-sm text-green-600">✓ Valid National ID ({formData.nationalId.length} digits)</p>
          )}
        </div>
            ...
            {formData.phoneNumber && formData.phoneNumber !== '+257 ' && (
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                <FontAwesomeIcon 
                  ...
                  className={`w-5 h-5 ${phoneValidation.isValid ? 'text-green-500' : 'text-red-500'}`} 
                />
              </div>
            )}
          </div>
          {formData.phoneNumber && formData.phoneNumber !== '+257 ' && !phoneValidation.isValid && (
            ...
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center space-x-2">
              {formData.password && (
                <FontAwesomeIcon 
                  ...
                  className={`w-5 h-5 ${passwordValidation.isValid ? 'text-green-500' : 'text-red-500'}`} 
                />
              )}
              <button
                type="button"
        ...
        <div>
            ...
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center space-x-2">
              {formData.confirmPassword && (
                <FontAwesomeIcon 
                  ...
                  className={`w-5 h-5 ${confirmPasswordValidation.isValid ? 'text-green-500' : 'text-red-500'}`} 
                />
              )}
              <button
                type="button"
          ...
          {formData.confirmPassword && !confirmPasswordValidation.isValid && (
            <p className="mt-1 text-sm text-red-600">{confirmPasswordValidation.message}</p>
          )}
          {formData.confirmPassword && confirmPasswordValidation.isValid && (
